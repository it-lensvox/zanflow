...
export function useDashboard() {
  ...
  // ── Load persisted date range preference on mount
  useEffect(() => {
    dashboardApi.getPreferences()
      .then(prefs => {
        const valid = ['7d', '30d', '90d', 'all'] as const;
        const saved = prefs?.dashboard_date_range as typeof valid[number];
        if (saved && valid.includes(saved)) setDateRange(saved);
      })
      .catch(() => { /* silently fall back to default 'all' */ })
      .finally(() => setPrefsLoaded(true));
  }, []);

  // ── Persist date range preference whenever the user changes it
  const handleSetDateRange = useCallback((range: '7d' | '30d' | '90d' | 'all') => {
    setDateRange(range);
    dashboardApi.savePreferences({ dashboard_date_range: range }).catch(() => {
      // Non-critical — UI already updated; silently ignore save failures
    });
  }, []);
 ...
 const allTasks: DashboardTask[] =
  ...
  // ── Filtered tasks for Overview tab (respects date range; allTasks is kept raw for Analytics/Today tabs)
  const filteredTasks = rangeStart
    ? allTasks.filter(t => {
        const dt = new Date(t.updated_at || t.created_at || t.start_date || '');
        return !isNaN(dt.getTime()) && dt >= rangeStart;
      })
    : allTasks;
  ...
  const totalTasks = rangeStart ? filteredTasks.length : totalTasksCount;
  const completedTasks = filteredTasks.filter(t => t.status === 'completed' || t.status === 'deployed').length;
  const overdueTasks = filteredTasks.filter(isTaskOverdue).length;
  const pendingTasks = filteredTasks.filter(t => t.status === 'pending' || t.status === 'backlog').length;
  ...
  const donut = [
    { label: 'Overdue',     value: filteredTasks.filter(t => isTaskOverdue(t)).length,                                                                                              color: '#EF4444' },
    { label: 'In Progress', value: filteredTasks.filter(t => t.status === 'in_progress' && !isTaskOverdue(t)).length,                                                              color: '#6366F1' },
    { label: 'Pending',     value: filteredTasks.filter(t => (t.status === 'pending' || t.status === 'backlog') && !isTaskOverdue(t)).length,                                      color: '#F59E0B' },
    { label: 'Completed',   value: filteredTasks.filter(t => t.status === 'completed' || t.status === 'deployed').length,                                                          color: '#22C55E' },
    { label: 'Deferred',    value: filteredTasks.filter(t => t.status === 'deferred' && !isTaskOverdue(t)).length,                                                                 color: '#9CA3AF' },
    { label: 'Review',      value: filteredTasks.filter(t => t.status === 'review' && !isTaskOverdue(t)).length,                                                                   color: '#8B5CF6' },
  ].filter(d => d.value > 0);
  const donutTotal = donut.reduce((s, d) => s + d.value, 0);

  // ── My Tasks 
  const myTasks = filteredTasks.filter(t => user?.id && (t.assigned_to || []).some(id => String(id) === String(user.id)));
  ...
  // ── Recent Activity (uses filteredDocs so it respects the date range)
  const recentActivity = [...filteredDocs]
    .sort((a, b) => new Date(b.updated_at || '').getTime() - new Date(a.updated_at || '').getTime())
    .slice(0, 6);

  // ── Projects Overview 
  const projectsOverview = projects.slice(0, 5).map((p, i) => {
    const pt = filteredTasks.filter(t =>
  ...
  return {
    ...
    // filters
    dateRange, setDateRange: handleSetDateRange, showRangePicker, setShowRangePicker,
    prefsLoaded,
