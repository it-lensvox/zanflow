...
import { CreateTeamModal } from '@/pages/TeamManagement/Teams/components/Createteammodal';
...
function MemberListContent({ roomId, roomType }: { roomId: string; roomType: 'team' | 'project' }) {
  ...
  return (
    <div className="space-y-1">
      {members.map((member) => (
        <div key={member.user.id} className="flex items-center gap-2 p-2 rounded hover:bg-accent transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-foreground truncate">{member.user.full_name || member.user.username}</p>
...
function ToastNotificationComponent({
  ...
  return (
    <div className="flex items-start gap-3 p-4 bg-card rounded-lg shadow-lg border border-border min-w-[300px] max-w-[400px] animate-slide-in">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-sm text-foreground">{toast.sender_name}</p>
            <p className="text-xs text-muted-foreground mt-0.5 truncate">{toast.message_preview}</p>
      </div>
      <button
...
export function TeamChatModern() {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] overflow-hidden px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 2xl:px-40 pt-6 pb-8 bg-background">
      <div className="flex flex-1 w-full bg-card rounded-xl shadow-sm border border-border overflow-hidden relative">
      ...
      {/* Left Sidebar */}
      <div className={cn(
        "bg-muted border-r border-border flex flex-col h-full overflow-hidden flex-shrink-0 transition-all duration-200",
        "w-full md:w-80",
        ...
        <div className="h-14 px-4 flex items-center justify-between bg-card border-b border-border">
          <h2 className="font-semibold text-base text-foreground">Chat</h2>
        ...
        {/* Search Bar */}
        <div className="px-3 py-3 bg-card border-b border-border">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search"
              className="w-full pl-9 pr-3 py-2 text-sm bg-background text-foreground placeholder:text-muted-foreground rounded border-none focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        </div>
        ...
        >
          {/* Tab Pills */}
         <Tabs.List className="flex items-center gap-1 px-3 py-2 bg-card border-b border-border">
            {tabConfig.map(({ value, label, count }) => (
              <div key={value} className="relative inline-flex">
                <Tabs.Trigger
                  value={value}
                  className="px-4 py-1.5 text-xs font-medium rounded-full transition-all data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=inactive]:text-muted-foreground data-[state=inactive]:hover:bg-accent"
                >
                  {label}
                </Tabs.Trigger>
                {count > 0 && (
          ...
          {/* Scrollable Lists */}
          <div className="flex-1 overflow-y-auto scrollbar-hide min-h-0 bg-card">
                    ...
                    {chat.filteredUsers.map(user => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "w-full px-4 py-3 flex items-center gap-3 hover:bg-accent transition-colors border-l-2",
                            isSelected ? "bg-blue-500/10 border-blue-500" : "border-transparent"
                          )}
                        >
                          <div className="relative flex-shrink-0">
                            <div className={cn(
                              "h-10 w-10 rounded-full flex items-center justify-center font-semibold text-sm overflow-hidden",
                              isSelected ? "bg-blue-600 text-white" : "bg-blue-500/15 text-blue-500"
                            )}>
                              {user.avatar ? (
                                <img
                                  src={user.avatar}
              ...
              <div className="bg-white">
                ...
                {chat.isLoadingProjects ? (
                  ...
                  <>
                    ...
                    {chat.projectRooms.length === 0 ? (
                      <div className="px-4 pb-3 text-xs text-muted-foreground">No projects</div>
                    ) : (
                      chat.projectRooms.map(project => {
                        const isSelected = chat.selectedProjectRoom?.id === project.id;
                        const unreadCount = chat.unreadCounts.get(project.id) || 0;
                ...
                {chat.isLoadingTeams ? (
                  ...
                  <>
                    ...
                    {chat.teamRooms.length === 0 ? (
                      <div className="px-4 pb-3 text-xs text-muted-foreground">No teams</div>
                    ) : (
                      chat.teamRooms.map(team => {
                        const isSelected = chat.selectedTeamRoom?.id === team.id;
                        const unreadCount = chat.unreadCounts.get(team.id) || 0;
              ...
              <div className="bg-white">
                <div className="border-t border-gray-100">
                  {chat.allUnreadItems.length === 0 ? (
                    <div className="p-4 text-center text-sm text-muted-foreground">No unread messages</div>
                  ) : (
                    chat.allUnreadItems.map(item => (
                      <button
                        ...
                        className={cn(
                          "w-full px-4 py-3 flex items-center gap-3 hover:bg-accent transition-colors border-l-2",
                          item.isSelected ? "bg-blue-500/10 border-blue-500" : "border-transparent"
                        )}
                      >
                        <div className="relative flex-shrink-0">
                          <div className={cn(
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          ...
                          <div className="flex items-center justify-between">
                            <p className="text-xs font-semibold text-foreground truncate">{item.preview}</p>
                            {item.unreadCount > 0 && (
                              <span className="ml-2 flex-shrink-0 h-5 min-w-[20px] px-1.5 bg-blue-600 text-white text-[10px] font-semibold rounded-full flex items-center justify-center">
                                {item.unreadCount}
                              </span>
      ...
      <div className={cn(
        "flex-1 flex flex-col bg-card h-full overflow-hidden min-w-0",
        !(chat.activeRoom || chat.selectedProjectRoom || chat.selectedTeamRoom)
          ? "hidden md:flex"
          : "flex"
      )}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                   <div className="flex items-center gap-2 min-w-0">
                    ...
                    {chat.selectedProjectRoom && <p className="text-xs text-muted-foreground" />}
                    {chat.selectedTeamRoom && <p className="text-xs text-muted-foreground" />}
                  </div>
                </div>
                    ...
                    <div className="absolute right-0 top-full mt-1 w-48 bg-popover rounded-lg shadow-lg border border-border py-1 z-50">
                      ...
                      {(chat.selectedTeamRoom || chat.selectedProjectRoom) && (
                        <>
                          ...
                          {chat.showMemberList && (
                            <div className="border-t border-border mt-1 pt-2 px-2 max-h-64 overflow-y-auto">
            ...
            {chat.headerView === 'chat' ? (
              <>
                ...
                <div
                  ...
                  className="flex-1 overflow-y-auto bg-background scrollbar-hide p-6"
                          ...
                          <div key={message.id}>
                            {showDateSeparator && (
                              <div className="flex items-center gap-3 my-4 px-2">
                                ...
                                <span className="text-[11px] font-medium text-muted-foreground bg-muted px-3 py-1 rounded-full whitespace-nowrap select-none">
                              ...
                              <div className={cn("flex-1 max-w-[65%]", isOwn ? "flex flex-col items-end" : "flex flex-col items-start")}>
                                {showAvatar && (
                                  <div className={cn("flex items-baseline gap-2 mb-1", isOwn ? "flex-row-reverse" : "")}>
                                   <span className={cn("text-xs font-medium", isOwn ? "text-muted-foreground" : "text-foreground")}>
                                ...
                                <div className="relative">
                                  ...
                                  {(isHovered || menuOpen) && (
                                    <div className={cn(
                                      ...
                                      <button onClick={() => chat.handleQuickReaction(message.id, '👍')} className="p-1 hover:bg-accent rounded transition-colors" title="Like">
                                        <span className="text-xs">👍</span>
                                      </button>
                                      <button onClick={() => chat.handleQuickReaction(message.id, '❤️')} className="p-1 hover:bg-accent rounded transition-colors" title="Love">
                                        <span className="text-xs">❤️</span>
                                      </button>
                                     <button onClick={() => chat.handleQuickReaction(message.id, '😊')} className="p-1 hover:bg-accent rounded transition-colors" title="Smile">
                                      ...
                                      {menuOpen && (
                                        <div
                                          ref={chat.menuRef}
                                          className={cn(
                                           "absolute top-full mt-1 bg-popover border border-border rounded-lg shadow-lg py-1 w-48 z-50",
                                            isOwn ? "right-0" : "left-0"
                                          )}
                                        >
                                          <button onClick={() => chat.handleReplyWithQuote(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">
                                            <Reply className="h-4 w-4" /> Reply
                                          </button>
                                          <button onClick={() => chat.handleForward(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">
                                            <Forward className="h-4 w-4" /> Forward
                                          </button>
                                          <button onClick={() => chat.handleCopyLink(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">
                                            <Link2 className="h-4 w-4" /> Copy link
                                          </button>
                                          <button onClick={() => chat.handlePinMessage(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">
                                            <Pin className="h-4 w-4" /> Pin message
                                          </button>
                                          <button onClick={() => chat.handleSaveMessage(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">

                                            <Bookmark className="h-4 w-4" /> Save
                                          </button>
                                          <button onClick={() => chat.handleMarkAsUnread(message)} className="w-full px-3 py-2 text-left text-sm hover:bg-accent flex items-center gap-2 text-foreground">
                                  ...
                                  {reactions && reactions.size > 0 && (
                                    <div className={cn("flex gap-1 mt-1", isOwn ? "justify-end" : "")}>
                                      {Array.from(reactions.entries()).map(([emoji, count]) => (
                                         <span key={emoji} className="inline-flex items-center gap-1 px-2 py-0.5 bg-card border border-border rounded-full text-xs">
                                          <span>{emoji}</span>
              ...
              <div className="flex-1 overflow-y-auto bg-background p-6">
                <div className="max-w-4xl mx-auto">
                   <h3 className="text-lg font-semibold text-foreground mb-4">Shared Documents</h3>
                  {chat.sharedDocuments.length === 0 ? (
                    <div className="flex items-center justify-center h-64">
                      <div className="text-center">
                        <Paperclip className="h-12 w-12 text-muted-foreground/40 mx-auto mb-3" />
                        <p className="text-sm text-muted-foreground">No shared documents</p>
                        <p className="text-xs text-muted-foreground/60 mt-1">Documents shared in this chat will appear here</p>
                      </div>
                    </div>
                  ) : (
          ...
          <div className="flex-1 flex items-center justify-center bg-background">
            <div className="text-center max-w-sm">
             ...
             <h3 className="text-xl font-semibold text-foreground mb-2">Welcome to Chat</h3>
              <p className="text-muted-foreground text-sm">Select a user from the list to start messaging</p>
