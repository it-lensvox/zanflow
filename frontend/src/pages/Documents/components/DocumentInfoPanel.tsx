...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
   <div className="flex items-center justify-between" style={{ padding: '10px 0', borderBottom: '1px solid hsl(var(--border))' }}>
      ...
      <span style={{ fontSize: 13, color: 'hsl(var(--foreground))', fontWeight: 500, textAlign: 'right' as const, maxWidth: '60%' }}>{value}</span>
    </div>
  );
}
...
export function DocumentInfoPanel({ doc, onClose }: { doc: Document | null; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end pointer-events-none">
     <div ref={panelRef} className="pointer-events-auto h-full flex flex-col animate-in slide-in-from-right duration-300" style={{ width: 340, background: 'hsl(var(--card))', borderLeft: '1px solid hsl(var(--border))', boxShadow: '0 25px 50px -12px rgba(0,0,0,.25)' }}>
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid hsl(var(--border))', background: 'hsl(var(--muted))' }}>
          <div className="flex items-center gap-2"><Info className="w-4 h-4" style={{ color: '#4169FF' }} /><span style={{ fontSize: 14, fontWeight: 600, color: 'hsl(var(--foreground))' }}>Document Info</span></div>
          <button onClick={onClose} style={{ color: 'hsl(var(--muted-foreground))', background: 'none', border: 'none', cursor: 'pointer', padding: 6 }}><X className="w-4 h-4" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
          {rows.map((r, i) => (
            <div key={i} className="flex items-start gap-3 py-1.5" style={{ borderBottom: '1px solid hsl(var(--border))' }}>
              <div className="mt-0.5 flex-shrink-0">{r.icon}</div>
              <div className="flex-1 min-w-0">
                <div style={{ fontSize: 13, fontWeight: 600, color: 'hsl(var(--muted-foreground))', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: 2 }}>{r.label}</div>
                <div style={{ fontSize: 12, color: 'hsl(var(--foreground))' }}>{r.value}</div>
              </div>
            </div>
          ))}
          <DetailRow label="Tags" value={
            doc.labels && doc.labels.length > 0
              ? <div className="flex flex-wrap gap-1.5 justify-end">
                ...
                {doc.labels.length > 2 && <span className="rounded-md" style={{ padding: '4px 8px', fontSize: 12, fontWeight: 500, background: 'hsl(var(--muted))', color: 'hsl(var(--muted-foreground))' }}>+{doc.labels.length - 2}</span>}
              </div>
